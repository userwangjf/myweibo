package com.wangjf.MultImageView;

/**
 * Created by wangjf on 17-8-5.
 */
...
public class MultImageView extends LinearLayout {
    ...
    private final static String TAG = "MultImageView";
    ...
    public void setList(List<String> lists) throws IllegalArgumentException {
        Log.i(TAG,"setList(List<String> lists)" + lists.size());
        ...
        if (MAX_WIDTH > 0) {
            pxMoreWandH = (MAX_WIDTH - pxImagePadding * 2) / div; //解决右侧图片和内容对不齐问题
            pxOneMaxWandH = MAX_WIDTH * 2 / div;
...
//            addView(new View(getContext()));
    ...
    private View createImageView(int position, final boolean isMultiImage) {
        ...
        Glide.with(getContext()).load(url).placeholder(R.drawable.default_image)
